...
import {
  ...
  transition,
  trigger
} from "./chunk-TMPKRLCA.js";
import {
  DOCUMENT
} from "./chunk-OSFBIZP7.js";
import {
...
var CdkVirtualForOf = class _CdkVirtualForOf {
  ...
  constructor(_viewContainerRef, _template, _differs, _viewRepeater, _viewport, ngZone) {
    ...
    this._viewport.renderedRangeStream.pipe((0, import_operators4.takeUntil)(this._destroyed)).subscribe((range2) => {
...
var SingleBoxSharedResizeObserver = class {
  constructor(_box) {
    ...
    this._resizeSubject = new import_rxjs7.Subject();
  ...
  observe(target) {
    if (!this._elementObservables.has(target)) {
      this._elementObservables.set(target, new import_rxjs7.Observable((observer) => {
        ...
        (0, import_operators5.filter)((entries) => entries.some((entry) => entry.target === target)),
        // Share a replay of the last event so that subsequent calls to observe the same element
        // receive initial sizing info like the first one. Also enable ref counting so the
        // element will be automatically unobserved when there are no more subscriptions.
        (0, import_operators5.shareReplay)({
          bufferSize: 1,
          refCount: true
        }),
...
var MatPaginatedTabHeader = class _MatPaginatedTabHeader {
  ...
  constructor(_elementRef, _changeDetectorRef, _viewportRuler, _dir, _ngZone, _platform, _animationMode) {
    ...
    this._destroyed = new import_rxjs8.Subject();
    this._showPaginationControls = false;
    this._disableScrollAfter = true;
    this._disableScrollBefore = true;
    this._stopScrolling = new import_rxjs8.Subject();
    ...
    _ngZone.runOutsideAngular(() => {
      (0, import_rxjs8.fromEvent)(_elementRef.nativeElement, "mouseleave").pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe(() => {
        this._stopInterval();
      });
    });
  }
  ngAfterViewInit() {
    (0, import_rxjs8.fromEvent)(this._previousPaginator.nativeElement, "touchstart", passiveEventListenerOptions).pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe(() => {
      this._handlePaginatorPress("before");
    });
    (0, import_rxjs8.fromEvent)(this._nextPaginator.nativeElement, "touchstart", passiveEventListenerOptions).pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe(() => {
      this._handlePaginatorPress("after");
    });
  }
  ngAfterContentInit() {
    const dirChange = this._dir ? this._dir.change : (0, import_rxjs8.of)("ltr");
    const resize = this._sharedResizeObserver.observe(this._elementRef.nativeElement).pipe((0, import_operators6.debounceTime)(32), (0, import_operators6.takeUntil)(this._destroyed));
    const viewportResize = this._viewportRuler.change(150).pipe((0, import_operators6.takeUntil)(this._destroyed));
    const realign = () => {
    ...
    (0, import_rxjs8.merge)(dirChange, viewportResize, resize, this._items.changes, this._itemsResized()).pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe(() => {
      this._ngZone.run(() => {
  ...
  _itemsResized() {
    ...
    return this._items.changes.pipe(
      (0, import_operators6.startWith)(this._items),
      (0, import_operators6.switchMap)((tabItems) => new import_rxjs8.Observable((observer) => this._ngZone.runOutsideAngular(() => {
      ...
      (0, import_operators6.skip)(1),
      // Skip emissions where all the elements are invisible since we don't want
      // the header to try and re-render with invalid measurements. See #25574.
      (0, import_operators6.filter)((entries) => entries.some((e) => e.contentRect.width > 0 && e.contentRect.height > 0))
    );
  }
  ...
  _handlePaginatorPress(direction, mouseEvent) {
    ...
    this._stopInterval();
    (0, import_rxjs8.timer)(HEADER_SCROLL_DELAY, HEADER_SCROLL_INTERVAL).pipe((0, import_operators6.takeUntil)((0, import_rxjs8.merge)(this._stopScrolling, this._destroyed))).subscribe(() => {
      const {
        maxScrollDistance,
        distance
      } = this._scrollHeader(direction);
...
var MatTabBodyPortal = class _MatTabBodyPortal extends CdkPortalOutlet {
  constructor(componentFactoryResolver, viewContainerRef, _host, _document) {
    ...
    this._host = _host;
    this._centeringSub = import_rxjs8.Subscription.EMPTY;
    this._leavingSub = import_rxjs8.Subscription.EMPTY;
  }
  /** Set initial visibility or set up subscription for changing visibility. */
  ngOnInit() {
    super.ngOnInit();
    this._centeringSub = this._host._beforeCentering.pipe((0, import_operators6.startWith)(this._host._isCenterPosition(this._host._position))).subscribe((isCentering) => {
...
var MatTabBody = class _MatTabBody {
  ...
  constructor(_elementRef, _dir, changeDetectorRef) {
    this._elementRef = _elementRef;
    this._dir = _dir;
    this._dirChangeSubscription = import_rxjs8.Subscription.EMPTY;
    this._translateTabComplete = new import_rxjs8.Subject();
...
var MatTabGroup = class _MatTabGroup {
  ...
  constructor(_elementRef, _changeDetectorRef, defaultConfig, _animationMode) {
    ...
    this._tabsSubscription = import_rxjs8.Subscription.EMPTY;
    this._tabLabelSubscription = import_rxjs8.Subscription.EMPTY;
  ...
  _subscribeToAllTabChanges() {
    this._allTabs.changes.pipe((0, import_operators6.startWith)(this._allTabs)).subscribe((tabs) => {
  ...
  _subscribeToTabLabels() {
    ...
    this._tabLabelSubscription = (0, import_rxjs8.merge)(...this._tabs.map((tab) => tab._stateChanges)).subscribe(() => this._changeDetectorRef.markForCheck());
  }
...
var MatTabNav = class _MatTabNav extends MatPaginatedTabHeader {
  ...
  constructor(elementRef, dir, ngZone, changeDetectorRef, viewportRuler, platform, animationMode, defaultConfig) {
    super(elementRef, changeDetectorRef, viewportRuler, dir, ngZone, platform, animationMode);
    this._fitInkBarToContent = new import_rxjs8.BehaviorSubject(false);
    this.stretchTabs = true;
    this.disableRipple = false;
    this.color = "primary";
    this.disablePagination = defaultConfig && defaultConfig.disablePagination != null ? defaultConfig.disablePagination : false;
  ...
  ngAfterContentInit() {
    this._inkBar = new MatInkBar(this._items);
    this._items.changes.pipe((0, import_operators6.startWith)(null), (0, import_operators6.takeUntil)(this._destroyed)).subscribe(() => {
...
var MatTabLink = class _MatTabLink extends InkBarItem {
  ...
  constructor(_tabNavBar, elementRef, globalRippleOptions, tabIndex, _focusMonitor, animationMode) {
    ...
    _tabNavBar._fitInkBarToContent.pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe((fitInkBarToContent) => {
